...
export default function SocialGraphPage() {
  const [viewMode, setViewMode] = useState<'search' | 'network'>('search');
  ...
  const handleSearch = async ({
    ...
    setLoading(true);
    setShowSuggestions(false);
    setSuggestions([]); // Clear suggestions to prevent them from re-appearing
    if (page === 1) {
      setResults(null);
    }
    ...
    try {
      ...
      setLoading(false);
      setShowSuggestions(false); // Double check it is closed
    }
  };
        ...
        <>
          <div className="max-w-3xl mx-auto text-center mb-12">
            ...
            <div className="space-y-4 text-left">
              <div className="flex flex-wrap gap-3 items-start justify-center">
                ...
                <Button onClick={() => handleSearch({ page: 1 })} disabled={loading} className="h-11 px-8 text-white bg-stone-800 hover:bg-stone-700">
              ...
              {showFilters && (
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 bg-stone-50 border border-stone-200 rounded-lg p-4 mt-4">
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-medium text-stone-600">Author</label>
                    <Input
                      type="text"
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-medium text-stone-600">Recipient</label>
                    <Input
                      type="text"
          ...
          {!results && !loading && (
            <div className="flex flex-col items-center justify-center pb-20 text-stone-400">
              ...
              <p className="text-lg font-medium text-stone-600 mb-2">Start by typing a name above</p>
              <p className="text-sm mb-6 max-w-xl text-center text-stone-500">
                Enter a name to trace every mention across letters and diaries — even when the person appears under different titles or spellings.
              </p>
              <div className="text-sm flex flex-wrap justify-center gap-2 text-stone-500">
                ...
                {[
                  ...
                  <span key={name}>
                    ...
                    {i < arr.length - 1 && <span className="text-stone-300 mx-2">·</span>}
                  </span>
                ))}
              </div>
            </div>
